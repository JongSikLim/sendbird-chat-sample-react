...
import { getAllApplicationUsers, sortUsersAlphabetical } from '../utils';
...
const OpenChannelReportAMessageUserChannel = (props) => {
  ...
  const setupUser = async () => {
    ...
    const [users] = await getAllApplicationUsers(); // load users initially
      ...
      case 'operator_then_member_alphabetical':
        ...
        const updatedUsersList = alphabeticalUsers.sort((user) => {
          const isOperator = currentlyJoinedChannelOperators.find(
            (operator) => user.userId === operator.userId.substr(0, 2)
          );
          if (isOperator) {
            return -1;
          }
        ...
        break;
      case 'all':
        const [users, usersError] = await getAllApplicationUsers(sb);
      ...
      {channels.map((channel) => {
        ...
        return (
          <div key={channel.url} className='channel-list-item'>
            ...
            <button
              className='control-button'
              onClick={() => toggleReportModal(channel, 'Channel')}
            >
              <img
